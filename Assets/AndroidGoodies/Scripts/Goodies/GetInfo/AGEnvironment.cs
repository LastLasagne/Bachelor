namespace DeadMosquito.AndroidGoodies
{
	...
	public static class AGEnvironment
	{
		...
		public static string ExternalStorageState => EnvClassCallStatic<string>("getExternalStorageState");

		/// <summary>
		/// Returns whether the primary shared/external storage media is emulated.
		/// </summary>
		/// <value><c>true</c> if external storage is emulated; otherwise, <c>false</c>.</value>
		public static bool IsExternalStorageEmulated => EnvClassCallStatic<bool>("isExternalStorageEmulated");
		...
		public static bool IsExternalStorageRemovable => EnvClassCallStatic<bool>("isExternalStorageRemovable");
		...
		static string GetFileDirectory(string methodName, params object[] args)
		{
			...
			{
				Debug.LogWarning($"Failed to get directory {methodName} on {C.AndroidOsEnvironment}, Error: {e.Message}");
				return null;
			}
		}
		...
		static T EnvClassCallStatic<T>(string methodName, params object[] args)
		{
			...
			{
				Debug.LogWarning($"Failed to invoke {methodName} on {C.AndroidOsEnvironment}, Error: {e.Message}");
				return default(T);
			}
		}
		...
		static string GetStringProperty(string propertyName)
		{
			...
			{
				Debug.LogWarning("Could not get the property: " + propertyName + ". Check the device API level if the property is present, reason: " + e.Message);
				return string.Empty;
			}
		}
