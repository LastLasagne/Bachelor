namespace DeadMosquito.AndroidGoodies
{
	...
	public class AudioAttributes
	{
		...
		public enum Flags
		{
			...
			[PublicAPI]
			FlagAll = FlagAudibilityEnforced | FlagSecure | FlagSco | FlagBeacon | FlagHwAvSync | FlagHwHotWord | FlagBypassInterruptionPolicy | FlagBypassMute | FlagLowLatency | FlagDeepBuffer,
		...
		AudioAttributes(Usage usage, ContentType contentType, Flags flags)
		{
			...
			AJO = new AndroidJavaObject(C.AndroidMediaAudioAttributesBuilder).CallAJO("setUsage", (int)usage).CallAJO("setContentType", (int)contentType).CallAJO("setFlags", (int)flags).CallAJO("build");
		}
	}
}
