...
namespace DeadMosquito.AndroidGoodies
{
	...
	public static class AGCamera
	{
		...
		/// </summary>
		/// <value>
		///     <c>true</c>, if device has camera, <c>false</c> otherwise.
		/// </value>
		[PublicAPI]
		public static bool DeviceHasCamera => AGDeviceInfo.SystemFeatures.HasSystemFeature(AGDeviceInfo.SystemFeatures.FEATURE_CAMERA);

		/// <summary>
		/// Check if device has frontal camera
		/// </summary>
		/// <value>
		///     <c>true</c>, if device has frontal camera, <c>false</c> otherwise.
		/// </value>
		[PublicAPI]
		public static bool DeviceHasFrontalCamera => AGDeviceInfo.SystemFeatures.HasSystemFeature(AGDeviceInfo.SystemFeatures.FEATURE_CAMERA_FRONT);

		/// <summary>
		/// Check if device has camera with autofocus
		/// </summary>
		/// <value>
		///     <c>true</c>, if device has camera with autofocus, <c>false</c> otherwise.
		/// </value>
		[PublicAPI]
		public static bool DeviceHasCameraWithAutoFocus => DeviceHasCamera && AGDeviceInfo.SystemFeatures.HasSystemFeature(AGDeviceInfo.SystemFeatures.FEATURE_CAMERA_AUTOFOCUS);

		/// <summary>
		/// Check if device has camera with flashlight
		/// </summary>
		/// <value>
		///     <c>true</c>, if device has camera with flashlight, <c>false</c> otherwise.
		/// </value>
		[PublicAPI]
		public static bool DeviceHasCameraWithFlashlight => DeviceHasCamera && AGDeviceInfo.SystemFeatures.HasSystemFeature(AGDeviceInfo.SystemFeatures.FEATURE_CAMERA_FLASH);
		...
		public static void TakePhoto(Action<ImagePickResult> onSuccess, Action<string> onError, ImageResultSize maxSize = ImageResultSize.Original, bool shouldGenerateThumbnails = true)
		{
			...
			if (onSuccess == null)
			{
				throw new ArgumentNullException(nameof(onSuccess), "Success callback cannot be null");
			}
		...
		/// <param name="generatePreviewImages">Whether to generate preview images.</param>
		[PublicAPI]
