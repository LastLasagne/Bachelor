namespace DeadMosquito.AndroidGoodies
{
	...
	public static class AGGPS
	{
		...
		public sealed class Location
		{
			...
			public override string ToString()
			{
				return $"[Location: Latitude={Latitude}, Longitude={Longitude}, "
					+ $"HasAccuracy={HasAccuracy}, Accuracy={Accuracy}, "
					+ $"Timestamp={Timestamp}, "
					+ $"HasSpeed={HasSpeed}, Speed={Speed}, "
					+ $"HasBearing={HasBearing}, Bearing={Bearing}, "
					+ $"HasAltitude={HasAltitude}, Altitude={Altitude}, "
		...
		public static Location GetLastKnownGPSLocation()
		{
			...
			try
			{
				var locationAJO = AGSystemService.LocationService.Call<AndroidJavaObject>("getLastKnownLocation", GPS_PROVIDER);
		...
		public static void RequestLocationUpdates(long minTime, float minDistance, Action<Location> onLocationChangedCallback)
		{
			...
			try
			{
				AGUtils.RunOnUiThread(() => AGSystemService.LocationService.Call("requestLocationUpdates", GPS_PROVIDER, minTime, minDistance, _currentListener));
			}
			catch (Exception e)
			{
				if (Debug.isDebugBuild)
				{
					Debug.LogWarning("Failed to register for location updates. Current device probably does not have GPS. Please check if device has GPS before invoking this method. " + e.Message);
				}
			}
		}
		...
		class LocationListenerProxy : AndroidJavaProxy
		{
			...
			void onLocationChanged( /*Location or List<Location> */
				AndroidJavaObject locationAJO
			)
			{
...
#pragma warning restore 0618
		...
		static Location LocationFromAJO( /*Location*/
			AndroidJavaObject locationAJO
		)
		{
			using (locationAJO)
