...
namespace DeadMosquito.AndroidGoodies
{
	...
	public static class AGSharedPrefs
	{
		...
		/// <param name="mode">Operating mode. Use 0 or MODE_PRIVATE for the default operation.</param>
		/// <returns>Returns true if the new values were successfully written to persistent storage.</returns>
		public static string GetString(string preferenceFileKey, string key, string defaultValue, int mode = MODE_PRIVATE)
		{
			if (AGUtils.IsNotAndroid())
			{
				return string.Empty;
		...
		static AndroidJavaObject GetSharedPrefs(string preferenceFileKey, int mode = MODE_PRIVATE) => AGUtils.Activity.CallAJO("getSharedPreferences", preferenceFileKey, mode);

		static AndroidJavaObject GetPrefs(int mode = MODE_PRIVATE) => AGUtils.Activity.CallAJO("getPreferences", mode);

		static AndroidJavaObject GetEditor(AndroidJavaObject prefs) => prefs.CallAJO("edit");

		static AndroidJavaObject GetEditorInternal(string perferenceFileKey, int mode = MODE_PRIVATE) => GetEditor(GetSharedPrefs(perferenceFileKey, mode));

		static bool Commit(AndroidJavaObject editor) => editor.CallBool("commit");
