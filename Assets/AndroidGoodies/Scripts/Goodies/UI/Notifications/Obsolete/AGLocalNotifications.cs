namespace DeadMosquito.AndroidGoodies
{
	...
	public static class AGLocalNotifications
	{
		...
		[PublicAPI]
		[Obsolete("Use AGNotificationManager class")]
		public static void ShowNotification([NotNull] string title, string text, DateTime? when, string tickerText = null, string iconName = null)
		{
			...
			using (var c = new AndroidJavaClass(C.GoodiesNotificationManagerClass))
			{
				...
				c.CallStatic("setNotification", AGUtils.Activity, new Random().Next(), when.Value.ToMillisSinceEpoch() - DateTime.Now.ToMillisSinceEpoch(), title, text, text, 1, 1, 1, "", "notify_icon_small", bgColor.ToAndroidColor(), PackageName);
			}
		}
		...
		public static void ShowNotification(int id, DateTime when, string title, string message, Color32 bgColor, bool sound = true, bool vibrate = true, bool lights = true, string bigIcon = "")
		{
			...
			using (var c = new AndroidJavaClass(C.GoodiesNotificationManagerClass))
			{
				c.CallStatic("setNotification", AGUtils.Activity, id, when.ToMillisSinceEpoch() - DateTime.Now.ToMillisSinceEpoch(), title, message, message, sound ? 1 : 0, vibrate ? 1 : 0, lights ? 1 : 0, bigIcon, "notify_icon_small", bgColor.ToAndroidColor(), PackageName);
			}
		}
		...
		public static void ShowNotificationRepeating(int id, DateTime when, long intervalMillis, string title, string message, Color32 bgColor, bool sound = true, bool vibrate = true, bool lights = true, string bigIcon = "")
		{
			...
			using (var c = new AndroidJavaClass(C.GoodiesNotificationManagerClass))
			{
				c.CallStatic("setRepeatingNotification", AGUtils.Activity, id, when.ToMillisSinceEpoch() - DateTime.Now.ToMillisSinceEpoch(), title, message, message, intervalMillis, sound ? 1 : 0, vibrate ? 1 : 0, lights ? 1 : 0, bigIcon, "notify_icon_small", bgColor.ToAndroidColor(), PackageName);
			}
		}
