namespace DeadMosquito.AndroidGoodies
{
	...
	public static class AGWallpaperManager
	{
		...
		static AndroidJavaObject WallpaperManager => C.AndroidAppWallpaperManager.AJCCallStaticOnceAJO("getInstance", AGUtils.Activity);
		...
		/// </param>
		public static void SetWallpaper([NotNull] Texture2D wallpaperTexture, AndroidRect visibleCropHint = null, bool allowBackup = true, WallpaperType which = WallpaperType.Unspecified)
		{
			if (wallpaperTexture == null)
			{
				throw new ArgumentNullException(nameof(wallpaperTexture));
			}
		...
		/// Which wallpaper to configure with the new imagery.
		/// </param>
		public static void SetWallpaper([NotNull] string imagePath, AndroidRect visibleCropHint = null, bool allowBackup = true, WallpaperType which = WallpaperType.Unspecified)
		{
			if (imagePath == null)
			{
		...
		static void SetWallpaperBitmap(AndroidJavaObject bitmapAjo, AndroidRect visibleCropHint = null, bool allowBackup = true, WallpaperType which = WallpaperType.Unspecified)
		{
			if (Check.IsSdkGreaterOrEqual(AGDeviceInfo.VersionCodes.N))
			{
				WallpaperManager.CallInt(SetBitmapMethodName, bitmapAjo, visibleCropHint?.ajo, allowBackup, (int)which);
			}
			else
			{
		...
		/// Which wallpaper to configure with the new imagery.
		/// </param>
		public static void ShowCropAndSetWallpaperChooser([NotNull] Texture2D wallpaperTexture, AndroidRect visibleCropHint = null, bool allowBackup = true, WallpaperType which = WallpaperType.Unspecified)
		{
			if (wallpaperTexture == null)
			{
				throw new ArgumentNullException(nameof(wallpaperTexture));
		...
		/// Which wallpaper to configure with the new imagery.
		/// </param>
		public static void ShowCropAndSetWallpaperChooser([NotNull] string imagePath, AndroidRect visibleCropHint = null, bool allowBackup = true, WallpaperType which = WallpaperType.Unspecified)
		{
			if (imagePath == null)
			{
				throw new ArgumentNullException(nameof(imagePath));
