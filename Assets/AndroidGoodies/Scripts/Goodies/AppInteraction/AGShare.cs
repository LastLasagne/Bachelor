...
namespace DeadMosquito.AndroidGoodies
{
	...
	public static class AGShare
	{
		...
		public static void ShareText(string subject, string body, bool withChooser = true, string chooserTitle = "Share via...")
		{
			...
			var intent = new AndroidIntent(AndroidIntent.ActionSend).SetType(AndroidIntent.MIMETypeTextPlain);
		...
		public static void ShareTextWithImage(string subject, string body, Texture2D image, bool withChooser = true, string chooserTitle = "Share via...")
		{
			...
			var intent = new AndroidIntent().SetAction(AndroidIntent.ActionSend).SetType(AndroidIntent.MIMETypeImageJpeg).PutExtra(AndroidIntent.ExtraSubject, subject).PutExtra(AndroidIntent.ExtraText, body);
		...
		public static void ShareScreenshot(bool withChooser = true, string chooserTitle = "Share via...")
		{
			...
			{
				var intent = new AndroidIntent().SetAction(AndroidIntent.ActionSend).SetType(AndroidIntent.MIMETypeImageJpeg);
		...
		static AndroidIntent CreateEmailIntent(string[] recipients, string subject, string body, Texture2D attachment = null, string[] cc = null, string[] bcc = null)
		{
			var uri = AndroidUri.Parse("mailto:");
			var intent = new AndroidIntent().SetAction(AndroidIntent.ActionSendTo).SetData(uri).PutExtra(AndroidIntent.ExtraEmail, recipients).PutExtra(AndroidIntent.ExtraSubject, subject).PutExtra(AndroidIntent.ExtraText, body);
			if (cc != null)
			{
		...
		public static void Tweet(string tweet, Texture2D image = null)
		{
			...
			{
				var intent = new AndroidIntent(AndroidIntent.ActionSend).SetType(AndroidIntent.MIMETypeTextPlain).PutExtra(AndroidIntent.ExtraText, tweet).SetPreferredActivity(TwitterPackage, "composer");
		...
		public static void ShareVideo(string videoPathOnDisc, string title, string chooserTitle = "Share Video")
		{
			...
			AGUtils.RunOnUiThread(
				() =>
					AndroidPersistanceUtilsInternal.ScanFile(
						videoPathOnDisc,
						(path, uri) =>
						{
							var intent = new AndroidIntent(AndroidIntent.ActionSend);
		...
		public static void SendTextMessageGeneric(string text, Func<bool> isInstalled, string package)
		{
			if (isInstalled())
			{
				var intent = new AndroidIntent(AndroidIntent.ActionSend).SetType(AndroidIntent.MIMETypeTextPlain).PutExtra(AndroidIntent.ExtraText, text).SetPackage(package);

				AGUtils.StartActivity(intent.AJO);
			}
			else
			{
				Debug.Log($"Can't send message because {package} is not installed");
			}
		}
		...
		public static void SendImageGeneric(Texture2D image, Func<bool> isInstalled, string package)
		{
			if (isInstalled())
			{
				var intent = new AndroidIntent(AndroidIntent.ActionSend).SetType(AndroidIntent.MIMETypeImageAll).SetPackage(package);
			...
			{
				Debug.Log($"Can't send image because {package} is not installed");
			}
		}
