namespace DeadMosquito.AndroidGoodies
{
	...
	public static class AGSettings
	{
		...
		public static void OpenApplicationDetailsSettings(string package)
		{
			...
			intent.SetData(AndroidUri.Parse($"package:{package}"));
			if (intent.ResolveActivity())
			{
				AGUtils.StartActivity(intent.AJO);
			}
		...
		/// <summary>
		/// Sets the system screen brightness. The value must be between 0 and 1 and will be clamped if it is not.
		///
		...
		public static void SetSystemScreenBrightness(float newBrightness)
		{
			...
			{
				Debug.LogError("The application does not have the permission to modify system settings." + " Check before invoking this method by invoking 'CanWriteSystemSettings()' and use 'OpenModifySystemSettingsActivity()' to prompt the use to allow");
				return;
			}

			try
			{
				AndroidSettings.System.PutInt(AndroidSettings.System.SCREEN_BRIGHTNESS_MODE, AndroidSettings.System.SCREEN_BRIGHTNESS_MODE_MANUAL);
				var brightnessInt = (int)(newBrightness * 255);
		...
		static void SetWindowScreenBrightness(float newBrightness)
		{
			...
			layout.SetStatic("screenBrightness", newBrightness);
