namespace DeadMosquito.AndroidGoodies
{
	...
	public static class AGPrintHelper
	{
		...
		public static void PrintImage([NotNull] Texture2D image, [NotNull] string jobName, Action onFinishCallback = null, ColorModes colorMode = ColorModes.Color, Orientations orientation = Orientations.Portrait, ScaleModes scaleMode = ScaleModes.Fit)
		{
			if (AGUtils.IsNotAndroid())
			{
				return;
		...
		public static void PrintHtmlPage([NotNull] string htmlText, [NotNull] string jobName)
		{
			...
			if (htmlText == null)
			{
				throw new ArgumentNullException(nameof(htmlText));
			}
		...
		public static void OnPrintSuccess() => _onPrintSuccess?.Invoke();
	}
}
