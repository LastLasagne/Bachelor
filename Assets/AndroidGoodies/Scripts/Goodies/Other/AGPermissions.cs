namespace DeadMosquito.AndroidGoodies
{
	...
	public static class AGPermissions
	{
		...
		public static bool IsPermissionGranted(string permission)
		{
			...
			{
				throw new ArgumentException("Permission must not be null or empty", nameof(permission));
			}
			...
			catch (Exception ex)
			{
				...
				{
					Debug.LogWarning("Could not check if runtime permission is granted. Check if Android version is 6.0 (API level 23) or higher. " + ex.Message);
				}
		...
		public static void RequestPermissions(string[] permissions, Action<PermissionRequestResult[]> onRequestPermissionsResults)
		{
			...
			{
				throw new ArgumentException("Requested permissions array must not be null or empty", nameof(permissions));
			}

			if (onRequestPermissionsResults == null)
			{
				throw new ArgumentNullException(nameof(onRequestPermissionsResults), "Listener cannot be null");
			}
		...
		{
			var result = AGDeviceInfo.SDK_INT < AGDeviceInfo.VersionCodes.M;
			if (result)
			{
		...
		[PublicAPI]
		public const string INSTANT_APP_FOREGROUND_SERVICE = "com.android.launcher.permission.INSTANT_APP_FOREGROUND_SERVICE";
		...
		public static void ExecuteIfHasPermission(string permission, [NotNull] Action action, [NotNull] Action onPermissionDenied)
		{
			...
			{
				throw new ArgumentNullException(nameof(onPermissionDenied));
			}
