	...
	class GoodiesSceneHelper : MonoBehaviour
	{
		...
		static void Init()
		{
			...
			lock (InitLock)
			{
				...
				var instances = FindObjectsByType<GoodiesSceneHelper>(FindObjectsSortMode.None);

				if (instances.Length > 1)
				{
					Debug.LogError(typeof(GoodiesSceneHelper) + " Something went really wrong " + " - there should never be more than 1 " + typeof(GoodiesSceneHelper) + " Reopening the scene might fix it.");
				}
				else if (instances.Length == 0)
				{
					...
					Debug.Log("[Singleton] An _instance of " + typeof(GoodiesSceneHelper) + " is needed in the scene, so '" + singleton.name + "' was created with DontDestroyOnLoad.");
				}
				else
				{
