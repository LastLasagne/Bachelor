namespace DeadMosquito.AndroidGoodies.Internal
{
	...
	public static class LocationUtils
	{
		public static void ComputeDistanceAndBearing(double lat1, double lon1, double lat2, double lon2, float[] results)
		{
			...
			if (results.Length > 1)
			{
				var initialBearing = (float)Math.Atan2(cosU2 * sinLambda, cosU1 * sinU2 - sinU1 * cosU2 * cosLambda);
				initialBearing *= 180.0f / (float)Math.PI;
				results[1] = initialBearing;
				if (results.Length > 2)
				{
					var finalBearing = (float)Math.Atan2(cosU1 * sinLambda, -sinU1 * cosU2 + cosU1 * sinU2 * cosLambda);
