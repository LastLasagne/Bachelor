namespace DeadMosquito.AndroidGoodies.Internal
{
	...
	public static class AndroidAlarmManager
	{
		...
		{
			AGSystemService.AlarmService.Call("setExact", RTC_WAKEUP, CalcMillis(when), AndroidPendingIntent.GetBroadcast(intent.AJO, id));
		}
		...
		{
			AGSystemService.AlarmService.Call("set", RTC_WAKEUP, CalcMillis(when), AndroidPendingIntent.GetBroadcast(intent.AJO, id));
		}

		public static void SetRepeating(AndroidIntent intent, DateTime when, long intervalMillis, int id)
		{
			AGSystemService.AlarmService.Call("setRepeating", RTC_WAKEUP, CalcMillis(when), intervalMillis, AndroidPendingIntent.GetBroadcast(intent.AJO, id));
		}
