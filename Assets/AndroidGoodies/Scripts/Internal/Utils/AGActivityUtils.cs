namespace DeadMosquito.AndroidGoodies.Internal
{
	...
	public static class AGActivityUtils
	{
		...
		const string EXTRAS_ALLOW_MULTIPLE = "EXTRAS_ALLOW_MULTIPLE";
		...
		{
			StartAndroidGoodiesActivity(REQ_CODE_PICK_CONTACT, intent => { });
		}

		public static void PickPhotoFromGallery(ImageResultSize maxSize, bool shouldGenerateThumbnails, bool allowMultiple)
		{
			PickImage(REQ_CODE_PICK_IMAGE, maxSize, shouldGenerateThumbnails, allowMultiple);
		}

		public static void TakePhoto(ImageResultSize maxSize, bool shouldGenerateThumbnails)
		{
			PickImage(REQ_CODE_TAKE_PHOTO, maxSize, shouldGenerateThumbnails, false);
		}

		static void PickImage(int requestCode, ImageResultSize maxSize, bool shouldGenerateThumbnails, bool allowMultiple)
		{
		...
		{
			StartAndroidGoodiesActivity(REQ_CODE_PICK_AUDIO, _ => { });
		}

		public static void PickVideoDevice(bool generatePreviewImages)
		{
			StartAndroidGoodiesActivity(REQ_CODE_PICK_VIDEO_DEVICE, intent => intent.PutExtra(EXTRAS_GENERATE_PREVIEW_IMAGES, generatePreviewImages));
		}

		public static void PickVideoCamera(bool generatePreviewImages)
		{
			StartAndroidGoodiesActivity(REQ_CODE_PICK_VIDEO_CAMERA, intent => intent.PutExtra(EXTRAS_GENERATE_PREVIEW_IMAGES, generatePreviewImages));
		}

		public static void PickFile(string mimeType)
		{
			StartAndroidGoodiesActivity(REQ_CODE_PICK_FILE, intent => intent.PutExtra(EXTRAS_MIME_TYPE, mimeType));
		}

		public static void RequestPermissions(string[] permissions)
		{
			StartAndroidGoodiesActivity(REQ_CODE_PERMISSIONS, intent => intent.PutExtra(EXTRAS_PERMISSIONS, permissions));
		}
