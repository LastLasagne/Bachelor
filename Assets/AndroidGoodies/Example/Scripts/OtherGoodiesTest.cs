...
namespace AndroidGoodiesExamples
{
	...
	public class OtherGoodiesTest : MonoBehaviour
	{
		...
		public void OnVibrateWithAudioAttributes()
		{
			...
			var audioAttributes = new AudioAttributes.Builder().SetContentType(AudioAttributes.ContentType.Music).SetFlags(AudioAttributes.Flags.FlagAll).SetUsage(AudioAttributes.Usage.Alarm).Build();
...
#pragma warning disable 0618
			AGLocalNotifications.ShowNotification(NotificationId, when, title, message, Color.red, sound, vibrate, lights, bigIcon);
#pragma warning restore 0618
		...
		public void SendLocalPushNotificationRepeating()
		{
			...
			var bigIcon = "app_icon";
#pragma warning disable 0618
			AGLocalNotifications.ShowNotificationRepeating(NotificationId, when, intervalMillis, title, message, Color.red, sound, vibrate, lights, bigIcon);
#pragma warning restore 0618
		}
...
#pragma warning restore 0618
		...
		public void InstallApkFromSdCard()
		{
			...
			var filePath = Path.Combine(AGEnvironment.ExternalStorageDirectoryPath, AGEnvironment.DirectoryDownloads) + apkFileName;
		...
		{
			AGPermissions.ExecuteIfHasPermission(AGPermissions.READ_CONTACTS, PickContact, () => ExampleUtil.ShowPermissionErrorToast(AGPermissions.READ_CONTACTS));
		}
		...
		void PickContact()
		{
			AGContacts.PickContact(
				pickedContact =>
				{
					var msg = $"Picked contact: {pickedContact.DisplayName}, photo URI: {pickedContact.PhotoUri}, emails: {string.Join(",", pickedContact.Emails.ToArray())}, phones: {string.Join(",", pickedContact.Phones.ToArray())}";
		...
		public void OnPickGalleryImage()
		{
			...
			// if image is larger it will be downscaled to the max size proportionally
			var imageResultSize = ImageResultSize.Max512;
			AGGallery.PickImageFromGallery(ProcessImage, errorMessage => AGUIMisc.ShowToast("Cancelled picking image from gallery: " + errorMessage), imageResultSize, shouldGenerateThumbnails);
		}
		...
		{
			AGPermissions.ExecuteIfHasPermission(AGPermissions.CAMERA, TakePhoto, () => ExampleUtil.ShowPermissionErrorToast(AGPermissions.CAMERA));
		}
		...
		void TakePhoto()
		{
			...
			AGCamera.TakePhoto(
				...
				error => AGUIMisc.ShowToast("Cancelled taking photo from camera: " + error),
				imageResultSize,
				shouldGenerateThumbnails
			);
		}
		...
		public void OnPickVideoFileDevice()
		{
			...
			AGFilePicker.PickVideo(
				...
				error => AGUIMisc.ShowToast("Cancelled picking video file: " + error),
		...
		{
			AGPermissions.ExecuteIfHasPermission(AGPermissions.CAMERA, RecordVideo, () => ExampleUtil.ShowPermissionErrorToast(AGPermissions.CAMERA));
		}

		void RecordVideo()
		{
			AGCamera.RecordVideo(
				videoFile =>
				{
				...
				error => AGUIMisc.ShowToast("Cancelled recording video file: " + error),
				true
			);
		}
		...
		public void OnRequestPermissions()
		{
			...
			var permissions = new[] { AGPermissions.WRITE_CONTACTS, AGPermissions.CALL_PHONE, AGPermissions.ACCESS_FINE_LOCATION, AGPermissions.READ_CALENDAR };
			...
			AGPermissions.RequestPermissions(
				permissions,
				results =>
				{
		...
		{
			AGPermissions.ExecuteIfHasPermission(AGPermissions.RECORD_AUDIO, StartRecording, () => ExampleUtil.ShowPermissionErrorToast(AGPermissions.RECORD_AUDIO));
		}
		...
		[UsedImplicitly]
		public void OnGetClipboardText()
		{
			Debug.Log($"Clipboard text: {AGClipboard.GetClipboardText()}");
		}
