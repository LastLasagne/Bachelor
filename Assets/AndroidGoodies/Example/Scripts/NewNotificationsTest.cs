...
#pragma warning restore 0649
		...
		void Start()
		{
			...
			AGNotificationManager.SetOnScheduledNotificationReceivedListener(id => Debug.Log($"Scheduled notification with id {id} has arrived."));
		...
		public void LogNotificationParams()
		{
			...
			{
				var text = $"App was opened via notification with parameters: {AGNotificationManager.GetNotificationParameter(ParamKeyId)}, {AGNotificationManager.GetNotificationParameter(ParamKeyDate)}";
		...
		public void CreateNotificationChannel()
		{
			...
			var group = new NotificationChannelGroup(_groupIdInputField.text, RandomName()) { Description = "Android-Goodies test notification channel group" };
		...
		public void TrySetNotificationPolicy()
		{
			...
			{
				var policy = new AGNotificationManager.Policy(AGNotificationManager.Policy.PriorityCategory.Alarms, AGNotificationManager.Policy.PrioritySenders.Any, AGNotificationManager.Policy.PrioritySenders.Any);
		...
		public void CreateProgressBarNotification()
		{
			...
			var builder = CreateBaseNotificationBuilder().SetContentTitle("Progress Bar Notification, id:" + _notificationId).SetContentText("So progressive :)");
		...
		public void CreateScheduledNotification()
		{
			...
			var notification = CreateBaseNotificationBuilder().SetContentTitle("Scheduled one!").SetContentText("Id: " + _notificationId).Build();
		...
		public void CreateRepeatingNotification()
		{
			...
			var notification = CreateBaseNotificationBuilder().SetContentTitle("Repeating one!").SetContentText("I will repeat!").Build();
		...
		public void CreateMessagingNotification()
		{
			...
			var builder = CreateBaseNotificationBuilder().SetMessagingStyle(new Notification.MessagingStyle("You:").SetConversationTitle("Conversation Title").AddMessage(NewMessage("Hello!", null)).AddMessage(NewMessage("Sup?", null)).AddMessage(NewMessage("Hi! Not bad, you?", "Ella")));
...
#pragma warning disable 0618
			var builder = new Notification.Builder(channelId, new Dictionary<string, string> { { ParamKeyId, "id: " + _notificationId }, { ParamKeyDate, "date: " + DateTime.Now } })
				.SetGroup(groupId)
...
#pragma warning restore 0618
		...
		public void CreateDifferentNotifications()
		{
			...
			builder = CreateBaseNotificationBuilder().SetContentTitle("With large icon!").SetBadgeIconType(Notification.BadgeIcon.Large).SetLargeIcon(iconTexture).SetCategory(Notification.CategoryMessage);
			Notify(builder);

			//Notification with number (shown as badge on app icon)
			builder = CreateBaseNotificationBuilder().SetContentTitle("With number").SetNumber(20);
			Notify(builder);

			//Chronometer notification
			builder = CreateBaseNotificationBuilder().SetContentTitle("UsesChronometer").SetUsesChronometer(true);
			Notify(builder);

			//Notification with ticker and subtext
			builder = CreateBaseNotificationBuilder().SetContentTitle("With subtext and ticker").SetSubText("Some subtext").SetTicker("Accessibility text");
			Notify(builder);

			//Auto-canceling notification (20s)
			builder = CreateBaseNotificationBuilder().SetContentTitle("With timeout").SetTimeoutAfter(20L * 1000L);
			Notify(builder);

			//Ongoing notification
			builder = CreateBaseNotificationBuilder().SetContentTitle("Ongoing one").SetOngoing(true);
			Notify(builder);

			//One-time alerting scheduled repeating notification
			builder = CreateBaseNotificationBuilder().SetContentTitle("Alert once").SetOnlyAlertOnce(true);
			var time = DateTime.Now.AddSeconds(5);
			...
			{
				builder = CreateBaseNotificationBuilder().SetContentTitle("With button").AddAction(Notification.CreateOpenUrlAction("https://github.com/TarasOsiris/android-goodies-docs-PRO/wiki", "notify_icon_small", "Open URL"));
				Notify(builder);
			}
		...
		public void CreateBigPictureNotification()
		{
			...
			var builder = CreateBaseNotificationBuilder().SetBigPictureStyle(new Notification.BigPictureStyle().SetSummaryText("Summary Text").SetBigContentTitle("Big Picture Title").SetBigLargeIcon(iconTexture).SetBigPicture(bigPictureTexture));
			Notify(builder);
		...
		{
			var builder = CreateBaseNotificationBuilder().SetInboxStyle(new Notification.InboxStyle().SetBigContentTitle("5 new messages from Julia:").AddLine("Message 1").AddLine("Message 2").SetSummaryText("and +3 more"));
			Notify(builder);
		}
