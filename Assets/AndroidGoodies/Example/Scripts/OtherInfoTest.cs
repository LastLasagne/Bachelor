...
#pragma warning restore 0618
			...
			builder.AppendLine("Alarms full path? : " + AGEnvironment.GetExternalStoragePublicDirectoryPath(AGEnvironment.DirectoryAlarms));
			...
			builder.AppendLine("\n-------------Permissions-------------");
			builder.AppendLine("Has location permission? " + AGPermissions.IsPermissionGranted(AGPermissions.ACCESS_FINE_LOCATION));
			builder.AppendLine("Has read contacts permission? " + AGPermissions.IsPermissionGranted(AGPermissions.READ_CONTACTS));
			builder.AppendLine("Has access network state permission? " + AGPermissions.IsPermissionGranted(AGPermissions.ACCESS_NETWORK_STATE));
		...
		{
			var amsterdamLocation = new AGGPS.Location(GPSTest.AmsterdamLatitude, GPSTest.AmsterdamLongitude, false, 0, 0);
			var brusselsLocation = new AGGPS.Location(GPSTest.BrusselsLatitude, GPSTest.BrusselsLongitude, false, 0, 0);
			return $"Distance between Amsterdam and Brussels is approx: {amsterdamLocation.DistanceTo(brusselsLocation)} metres";
		}
	}
}
